...
export default function SolarDesigner() {
  ...
  useEffect(() => {
    ...
    map.on('moveend', () => { const c = map.getCenter(); setCurrentCenter({ lat: c.lat, lng: c.lng }) })
  ...
  function autoPlaceFromPoly(polyLngLat: number[][], mapInstance = mapRef.current, turf = (window as any).turf, limit?: number) {
    if (!polyLngLat || !mapInstance) return { placed: 0, poly: polyLngLat }
    ...
    const newPanels: any[] = []
    ;(window as any).panelLayers?.forEach((l: any) => mapInstance.removeLayer(l))
    ...
    for (let lng = bbox[0]; lng < bbox[2] && (limit ? count < limit : true); lng += stepX) {
      for (let lat = bbox[1]; lat < bbox[3] && (limit ? count < limit : true); lat += stepY) {
        ...
        if (turf.booleanPointInPolygon(pt, poly)) {
          ...
          layer.on('click', () => { mapInstance.removeLayer(layer); setPanels(prev => prev.filter(p => p.id !== id)) })
  ...
  // ADVANCED: Generate roof auto based on kW need
  const handleAutoDesign = (desiredKw: number, panelsNeeded: number, autoRoof: boolean) => {
    ...
    // If no roof drawn, create auto roof centered at currentCenter
    if (!targetPoly || autoRoof) {
      const areaPerPanel = panelW * panelH * 1.4 // with gap
      const totalAreaNeeded = panelsNeeded * areaPerPanel * 1.2 // 20% buffer
      const side = Math.sqrt(totalAreaNeeded) // square root
      ...
      // Create rectangle polygon [lng,lat]
      targetPoly = [
      ...
      // Set best tilt/az from Google insights
    ...
    // Place exact panels needed
    autoPlaceFromPoly(targetPoly!, mapRef.current, turf, panelsNeeded)

    // Update lead if selected
    if (selectedLead) {
      updateLead(selectedLead.id, { system_size_kw: desiredKw, panel_count: panelsNeeded })
    }
  }
  ...
  const selectResult = (item: any) => {
    ...
    // NEXT LEVEL: As soon as address selected, open wizard asking kW
    ...
    <div className="h-screen flex flex-col bg-[#f8fafc] text-gray-900 overflow-hidden">
      ...
      <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 shadow-sm">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button onClick={() => setShowWizard(true)} className="px-4 py-1.5 text-xs rounded-xl bg-gray-900 text-white font-bold shadow hover:bg-black animate-pulse">🤖 AI Auto Design</button>
          ...
          {user || (typeof window !== 'undefined' && localStorage.getItem('mock_user')) ? <UserMenu /> : <button onClick={() => router.push('/auth')} className="px-3 py-1.5 text-xs rounded-lg bg-gray-900 text-white font-semibold">Login →</button>}
        </div>
      </div>

      {typeof window !== 'undefined' && !localStorage.getItem('mock_user') && !user && (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[300px] bg-white border-r border-gray-200 flex flex-col hidden lg:flex shadow-sm">
          <div className="p-3 border-b border-gray-100">
            ...
            {solarInsights && (
              <div className="mt-3 bg-gray-50 border border-gray-200 rounded-xl p-2.5 text-[11px] space-y-1.5">
                ...
                <div className="text-gray-600">Max: <b>{solarInsights.solarPotential?.maxArrayPanelsCount}</b> panels • {solarInsights.solarPotential?.maxArrayAreaMeters2} m²</div>
                <div className="mt-2 space-y-1">
                  {solarInsights.solarPotential?.roofSegmentStats?.slice(0, 2).map((seg: any, i: number) => (
                    <div key={i} className={`p-2 rounded-lg border text-[11px] ${i === 0 ? 'bg-blue-50 border-blue-200 text-blue-900' : 'bg-white border-gray-200'}`}>
                      {i === 0 ? '★ BEST' : `Seg ${i + 1}`}: {seg.pitchDegrees?.toFixed(0)}° / {seg.azimuthDegrees?.toFixed(0)}° • {seg.stats?.areaMeters2?.toFixed(0)}m²
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2 bg-gray-50/50">
            {leads.map(lead => (
              <div key={lead.id} onClick={() => { setSelectedLead(lead); if (lead.lat) { mapRef.current?.setView([lead.lat, lead.lng], 19); fetchSolarInsights(lead.lat, lead.lng) } }} className={`bg-white border rounded-xl p-3 cursor-pointer shadow-sm hover:shadow transition ${selectedLead?.id === lead.id ? 'border-blue-500 ring-2 ring-blue-100' : 'border-gray-200'}`}>
        ...
        <div className="flex-1 relative bg-[#eef2f7] flex flex-col">
          <div className="absolute top-3 left-3 z-[500] flex gap-2">
            <div className="bg-white/90 backdrop-blur border border-gray-200 rounded-xl p-2 flex gap-2 shadow-lg">
              <button onClick={() => { if (!mapRef.current || !L) return; // @ts-ignore
                  new L.Draw.Polygon(mapRef.current, { allowIntersection: false, shapeOptions: { color: '#2563eb', weight: 3 } }).enable()
                }} className="bg-gray-900 text-white text-xs px-3.5 py-2 rounded-lg font-semibold hover:bg-black shadow">✏️ Draw Roof</button>
              ...
              <button onClick={() => setShowWizard(true)} className="bg-gradient-to-r from-blue-600 to-purple-600 text-white text-xs px-3.5 py-2 rounded-lg font-bold shadow animate-pulse">🤖 AI kW → Auto Design</button>
          ...
          <div className="absolute top-3 right-3 z-[500] w-[300px]">
            <div className="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
              <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="🔍 Address डालें - e.g. Vaishali Nagar Jaipur" className="w-full px-4 py-2.5 text-sm focus:outline-none placeholder:text-gray-400" />
          ...
          {shadeEnabled && mapRef.current && L && <ShadeOverlay map={mapRef.current} L={L} center={currentCenter} enabled={shadeEnabled} />}
            ...
            {activeTab === 'design' && (
              <>
                <div className="bg-gradient-to-r from-gray-900 to-gray-700 text-white rounded-xl p-4 mb-4">
                  <div className="text-xs text-gray-300 uppercase font-semibold">Current Design - Auto Generated</div>
                ...
                <div className="space-y-3">
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-800"><b>🤖 AI Mode:</b> Address → kW input → Roof auto-detect → {panelWatt}W panels auto-place → Full details generated. No manual drawing needed!</div>
                  <div><label className="text-xs font-medium text-gray-700">Panel: {panelWatt}W | Tilt {tilt}° | Az {azimuth}° | Shade {shading}%</label></div>
                </div>
              </>
            )}
            {activeTab === 'energy' && (
              <div>
                <div className="text-xs font-bold text-gray-500 uppercase mb-3">Monthly - {prod.systemKw.toFixed(2)}kW → {Math.round(prod.annualKwh).toLocaleString()} kWh/yr</div>
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                  <div className="flex items-end gap-1 h-32">{prod.monthly.map((v, i) => <div key={i} className="flex-1 bg-gradient-to-t from-blue-600 to-blue-400 rounded-t" style={{ height: `${(v / Math.max(...prod.monthly)) * 100}%` }} />)}</div>
                </div>
              </div>
            )}
            {activeTab === 'finance' && (
            ...
            {activeTab === 'market' && (
              <div className="-m-4">
                <JaipurMarketPanel onSelectQuote={(kw, sku, struct) => {
                  setPanelWatt(JAIPUR_PANELS.find(p=>p.sku===sku)?.watt || 540);
                  // calculate desired kw auto design trigger
                  const panelsNeeded = Math.ceil(kw*1000/(JAIPUR_PANELS.find(p=>p.sku===sku)?.watt || 540));
      ...
      {/* ROOF IMAGE UPLOAD DESIGNER - NEW FEATURE */}
      {showImageDesigner && (
        <RoofImageDesigner
          ...
          panelW={panelW}
          panelH={panelH}
          onDesignComplete={(count, area, imgUrl) => {
            setRoofArea(area)
            ...
            alert(`✅ Roof Image Design Applied! ${count} panels • ${area.toFixed(1)} m² • ${(count * panelWatt / 1000).toFixed(2)} kW`)
          }}
        />
      )}

      {/* ADVANCED TOOLS HUB - 5 FEATURES */}
      {showAdvanced && (
      ...
      {showProposal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[2000] flex items-center justify-center p-4" onClick={() => setShowProposal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-[800px] overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b flex justify-between"><div className="flex gap-3"><img src="/raavi-logo.png" className="h-10 w-auto" /><div><div className="font-bold">Raavi Solar</div><div className="text-xs text-gray-500">Proposal for {selectedLead?.name || 'Demo'} • 9214567383</div></div></div><div className="bg-gray-900 text-white px-4 py-1 rounded-full text-sm font-bold">⚡ {prod.systemKw.toFixed(2)}kW</div></div>
            <div className="p-6 bg-gray-50"><div className="grid grid-cols-2 gap-4 text-sm"><div className="bg-white border rounded-xl p-4">Panels: {panels.length} x {panelWatt}W<br/>Prod: {Math.round(prod.annualKwh).toLocaleString()} kWh/yr</div><div className="bg-green-50 border border-green-200 rounded-xl p-4">Net ₹{Math.round(finance.net).toLocaleString()} • Payback {finance.payback.toFixed(1)}yr</div></div><div className="mt-4 flex gap-2"><button onClick={() => window.print()} className="px-5 py-2.5 bg-gray-900 text-white rounded-xl text-sm">🖨️ Print PDF</button><button onClick={() => setShowProposal(false)} className="px-5 py-2.5 bg-white border rounded-xl text-sm">Close</button></div></div>
          </div>
        </div>
      )}
    </div>
